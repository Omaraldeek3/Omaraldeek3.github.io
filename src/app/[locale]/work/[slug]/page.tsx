...
export default async function ProjectPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <main id="main" className="project-page"><section className="project-hero wrap" id="top"><Link className="back-link" href={`/${locale}/lab/design-studies`}>← {t.backWork}</Link><div className="project-title-row"><div><span className="project-disclosure">{t.concept}</span><h1>{p.name}<span>.</span></h1><p>{p.summary}</p></div><div><span className="mono">{project.year} / {project.number}</span><p>{p.type}</p><div className="tags">{p.tags.map(tag => <span key={tag}>{tag}</span>)}</div></div></div></section><div className={`project-large-preview wrap ${project.theme}`}><SitePreview project={project} locale={locale} priority /></div><section className="case-study wrap"><span className="eyebrow">{t.projectDetails}</span><div><article><span className="mono">01 /</span><h2>{t.goal}</h2><p>{p.goal}</p></article><article><span className="mono">02 /</span><h2>{t.solution}</h2><p>{p.solution}</p></article><article><span className="mono">03 /</span><h2>{t.delivered}</h2><ul>{p.parts.map(part => <li key={part}>{part}</li>)}</ul></article></div></section><section className="preview-section wrap"><div className="preview-section-heading"><h2>{t.preview}</h2><Link className="button button-mint" href={`/${locale}/work/${slug}/preview`}>{t.openPreview}<Arrow /></Link></div><div className={`responsive-showcase ${project.theme}`}><div className="showcase-desktop"><SitePreview project={project} locale={locale} /></div><div className="showcase-mobile"><SitePreview project={project} locale={locale} /></div></div></section><section className="next-project wrap"><span className="eyebrow">{t.nextProject}</span><Link href={`/${locale}/work/${next.slug}`}>{next[locale].name}<Arrow /></Link></section></main>;
}
